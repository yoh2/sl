 ...
 *========================================
 */
/* sl version 5.03-1:add features of sl5-1.patch by Izumi 2000/06/24         */
/*                                              by yoh2           2017/12/05 */
...
#else
#  define USLEEP_ARG0_TYPE usecnds_t
#endif
...
void init_seed(void)
{
    ...
    s[0] = (unsigned short int)ts.tv_sec;
    s[1] = (unsigned short int)ts.tv_nsec;
    s[2] = (unsigned short int)(ts.tv_nsec >> 16);
    seed48(s);
}
...
int main(int argc, char *argv[])
{
    ...
    char num[10];
    char *pp;
    char *c[D51PATTERNS][D51HEIGHT+1];
    ...
    DIREC = RTOL;
    p = 3 * COLS / 10;
    pp = (char*)malloc((size_t)(COLS + ALL_LENGTH + 10) * (D51HEIGHT + 1) * (D51PATTERNS + 1));
    for (i = 0; i <= (COLS + ALL_LENGTH + 1) * (D51HEIGHT + 1) * D51PATTERNS; ++i) {
        pp[i] = '\0';
    }

    for (j = 0; j < D51PATTERNS; ++j) {
        for (i = 0; i <= D51HEIGHT; ++i) {
            c[j][i] = pp + (COLS + ALL_LENGTH + 2) * i + (COLS + ALL_LENGTH + 1) * (D51HEIGHT + 1) * j;
            for (k = 0; k < COLS; ++k) {
                strcat(c[j][i], " ");
            }
...
int add_D51_coach(int x, char *c[]) {
    ...
    for (i = 0; i <= D51HEIGHT; ++i) {
        my_mvaddstr(y + i, 0, c[(D51HEIGHT + 1) * ((ALL_LENGTH + x) % D51PATTERNS) + i] + COLS - x);
    }
    if (ACCIDENT == 1) {
        add_man(y + 2, x + 43);
        add_man(y + 2, x + 47);
